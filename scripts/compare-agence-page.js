...
for (const v of VIEWS) {
  for (const { label, suffix } of PAIRS) {
    const file = `agence-scroll-${suffix}.png`;
...
if (total) {
  console.log(`\nAgence: ${total - fails}/${total} PASS, avg ${(sum / total).toFixed(2)}%`);
}
