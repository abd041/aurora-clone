...
const cloneLabel = process.argv[2] || 'clone-rejoindre-v3';
...
for (const v of VIEWS) {
  for (const { label, suffix } of PAIRS) {
    const file = `nous-rejoindre-scroll-${suffix}.png`;
...
if (total) {
  console.log(`\nNous-rejoindre: ${total - fails}/${total} PASS, avg ${(sum / total).toFixed(2)}%`);
}
